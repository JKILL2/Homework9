...
--3. List the manager of each department with the following information (dept number, dept name, manager's employee number, last name, first name, start and end employment dates)
...
--4. List the department of each employee with the following information (employee number, last name, first name, and dept name)
...
--7. List all employees in the Sales and Development departments, including their employee number, last name, first name, and dept name.
SELECT emp.emp_no, emp.last_name, emp.first_name, dep.dept_name 
FROM employees emp 
JOIN dept_emp ON dept_emp.emp_no = emp.emp_no 
INNER JOIN departments dep ON dep.dept_no = dept_emp.dept_no
